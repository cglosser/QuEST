...
BOOST_FIXTURE_TEST_CASE(Construction, PointSetup)
{
  ...
  BOOST_CHECK_CLOSE(grid.max_diagonal, std::sqrt(5.0), 1e-16);

  BOOST_CHECK_EQUAL(boxes[0].second - boxes[0].first, 1);
  BOOST_CHECK_EQUAL(boxes[1].second - boxes[1].first, 0);
  BOOST_CHECK_EQUAL(boxes[2].second - boxes[2].first, 1);
  BOOST_CHECK_EQUAL(boxes[3].second - boxes[3].first, 1);
  BOOST_CHECK_EQUAL(boxes[4].second - boxes[4].first, 2);
  BOOST_CHECK_EQUAL(boxes[5].second - boxes[5].first, 2);

  BOOST_CHECK_EQUAL(grid.spatial_coord_of_box(0), Eigen::Vector3d(-1, -1, 0));
  BOOST_CHECK_EQUAL(grid.spatial_coord_of_box(1), Eigen::Vector3d(-1, 0, 0));
  BOOST_CHECK_EQUAL(grid.spatial_coord_of_box(2), Eigen::Vector3d(-1, 1, 0));
  BOOST_CHECK_EQUAL(grid.spatial_coord_of_box(3), Eigen::Vector3d(0, -1, 0));
  BOOST_CHECK_EQUAL(grid.spatial_coord_of_box(4), Eigen::Vector3d(0, 0, 0));
  BOOST_CHECK_EQUAL(grid.spatial_coord_of_box(5), Eigen::Vector3d(0, 1, 0));
}

BOOST_FIXTURE_TEST_CASE(SORT_POINTS, PointSetup)
{
  AIM::Grid grid(grid_spacing, dots, 0);
  ...
  BOOST_CHECK_EQUAL(grid_idx(dots->at(1).position()), 2);
  BOOST_CHECK_EQUAL(grid_idx(dots->at(2).position()), 3);
  BOOST_CHECK_EQUAL(grid_idx(dots->at(3).position()), 4);
